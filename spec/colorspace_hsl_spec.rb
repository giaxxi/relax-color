...
describe Relax::ColorSpace::Hsl do
  let(:my_color) { Relax::ColorSpace::Hsl.new(341, 100, 50) }

  it 'Instantiate a ColorSpace::Hsl object by default method' do
    expect(my_color).to be_a Relax::ColorSpace::Hsl
  end

  context 'Instance initialization errors' do
    it  'Raises ArgumentError when instantiating' \
        'an object with a wrong argument' do
      expect { Relax::ColorSpace::Hsl.new(10) }.to raise_error ArgumentError
    end
    it  'Raises ArgumentError when instantiating' \
        'an object with a wrong argument' do
      expect { Relax::ColorSpace::Hsl.new('a', 10, 10) }
        .to raise_error ArgumentError
    end
    it  'Raises ChannelsOutOfRange when instantiating' \
        'an object with a wrong argument' do
      expect { Relax::ColorSpace::Hsl.new(361, 0, 0) }
        .to raise_error Relax::Errors::Hsl::ChannelsOutOfRange
      expect { Relax::ColorSpace::Hsl.new(-1, 0, 0) }
        .to raise_error Relax::Errors::Hsl::ChannelsOutOfRange
      expect { Relax::ColorSpace::Hsl.new(1, 0, 101) }
        .to raise_error Relax::Errors::Hsl::ChannelsOutOfRange
    end
  end
end
...
describe Relax::ColorSpace::Hsl do
  ...
  context Relax::HslToRgb do
    it 'Converts properly, hsl to rgba (1)' do
      expect(Relax::ColorSpace::Hsl.new(341, 100, 50)
        .to_rgba).to eql [255, 0, 81]
    end
    it 'Converts properly, hsl to rgba (2)' do
      expect(Relax::ColorSpace::Hsl.new(100, 100, 50)
        .to_rgba).to eql [85, 255, 0]
    end
  end
end
